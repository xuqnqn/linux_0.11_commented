...
pg_dir:			;// 页目录将会存放在这里。
.globl startup_32	
startup_32:			;// 以下5行设置各个数据段寄存器。指向gdt数据段描述符项
	movl $0x10,%eax
...
;// 描述表.新的全局段描述表中初始内容与setup.s 中的完全一样。
	...
	mov %ax,%fs
	mov %ax,%gs
	lss stack_start,%esp		;// 表示_stack_start -> ss:esp，设置系统堆栈。
...
check_x87:
	...
	movl %eax,%cr0
	ret
.align 2				;// 这里".align 2"的含义是指存储边界对齐调整。即按4 字节方式对齐内存地址。
...
after_page_tables:
	...
	pushl $main		;// '_main'是编译程序对main 的内部表示方法。
	jmp setup_paging
L6:
...
;// 设置页目录基址寄存器cr3 的值，指向页目录表。
	xorl %eax,%eax		/* 页目录表(pg_dir) is at 0x0000 */
