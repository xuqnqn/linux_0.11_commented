...
# posterity.
	...
	mov	$INITSEG, %ax	;// 将ds 置成INITSEG(9000)。这已经在bootsect 程序中
	mov	%ax, %ds	;// 设置过，但是现在是setup 程序，Linus 觉得需要再重新设置一下。
	mov	$0x03, %ah	;// BIOS 中断10 的读光标功能号ah = 03
	xor	%bh, %bh	;// 输入：bh = 页号
	int	$0x10		;// 返回：ch = 扫描开始线，cl = 扫描结束线，
	mov	%dx, %ds:0	;// dh = 行号(00 是顶端)，dl = 列号(00 是左边)。
				;// 将光标位置信息存放在90000 处，控制台初始化时会来取。
# Get memory size (extended mem, kB)

	mov	$0x88, %ah 	;// 这3句取扩展内存的大小值（KB）。
	int	$0x15		;// 是调用中断15，功能号ah = 88
	mov	%ax, %ds:2	;// 返回：ax = 从100000（1M）处开始的扩展内存大小(KB)。
				;// 若出错则CF 置位，ax = 出错码。
...
do_move:
	mov	%ax, %es	;// es:di -> 目的地址(初始为0000:0)
	add	$0x1000, %ax
	cmp	$0x9000, %ax	;// 已经把从8000 段开始的64k 代码移动完？
	jz	end_move
	mov	%ax, %ds	;// ds:si -> 源地址(初始为1000:0)
	sub	%di, %di
	sub	%si, %si
...
end_move:
	...
	lidt	idt_48		;// 加载中断描述符表(idt)寄存器，idt_48 是6 字节操作数的位置
...
;// 以上的操作很简单，现在我们开启A20 地址线。
	...
	#mov	$0xD1, %al	;// D1 命令码-表示要写数据到8042 的P2 端口。P2 端
	#out	%al, $0x64	;// 口的位1 用于A20 线的选通。数据要写到60 口。
	#call	empty_8042	;// 等待输入缓冲器空，看命令是否被接受。
	#mov	$0xDF, %al	;// A20 on 选通A20 地址线的参数。
	#out	%al, $0x60
...
;// 所以我们就必须重新对8259 中断控制器进行编程，这一点都没劲。
	...
	mov	$0x11, %al		;// 11 表示初始化命令开始，是ICW1 命令字，表示边
					;// 沿触发、多片8259 级连、最后要发送ICW4 命令字。
	out	%al, $0x20		;// 发送到8259A 主芯片。
	...
	out	%al, $0x21		;// 送主芯片ICW2 命令字，起始中断号，要送奇地址。
	.word	0x00eb,0x00eb
	mov	$0x28, %al		# start of hardware int's 2 (0x28)
	out	%al, $0xA1		;// 送从芯片ICW2 命令字，从芯片的起始中断号。
	.word	0x00eb,0x00eb		#               IR 7654 3210
	mov	$0x04, %al		# 8259-1 is master(0000 0100) --\
	out	%al, $0x21		;// 送主芯片ICW3 命令字，主芯片的IR2 连从芯片INT。
	.word	0x00eb,0x00eb		;// 参见代码列表后的说明。
	mov	$0x02, %al		# 8259-2 is slave(       010 --> 2)
	out	%al, $0xA1		;// 送从芯片ICW3 命令字，表示从芯片的INT 连到主芯片的IR2 引脚上。
	.word	0x00eb,0x00eb
	mov	$0x01, %al		# 8086 mode for both
	out	%al, $0x21		;// 送主芯片ICW4 命令字。8086 模式；普通EOI 方式，需发送指令来复位。初始化结束，芯片就绪。
	.word	0x00eb,0x00eb
	out	%al, $0xA1		;// 送从芯片ICW4 命令字，内容同上。
	.word	0x00eb,0x00eb
...
empty_8042:
	...
	in	$0x64, %al	;// 读AT 键盘控制器状态寄存器。
	test	$2, %al	;// 测试位2，输入缓冲器满？
...
gdt_48:
	.word	0x800			# gdt limit=2048, 256 GDT entries 全局表长度为2k 字节，因为每8 字节组成一个段描述符项,所以表中共可有256 项。
	.word   512+gdt, 0x9		# gdt base = 0X9xxxx, 4 个字节构成的内存线性地址：0009<<16 + 0200+gdt, 也即90200 + gdt(即在本程序段中的偏移地址，205 行)。
